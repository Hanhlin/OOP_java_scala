...
abstract class Tree[+T] {                 // This definition should be roughly 3 lines, including closing "}"                   
...
case class Leaf[T] (x: T) extends Tree[T] {  // Roughly 4 lines
  override def toString() = "Leaf(" + x + ")"
  override def map[E] (f: T => E): Tree[E] = Leaf(f(x))
...
case class Node[T] (x: T, children: List[Tree[T]]) extends Tree[T]  { // Roughly 4 lines 
  override def toString(): String = "Node(" + x + ","+ children.toString() + ")"
  override def map[E] (f: T => E): Tree[E] = {
    Node(f(x), children.map(x => x.map(f)))
  }
  override def element() = x
}
...
class B(x:Int, y:Int) extends A(x) {
  ...
  override def toString(): String = "B(" + (this.value - v2) + "," + v2 + ")"
} // Roughly 4 lines
...
//   end
  ...
  def breadthFirst[T] (t: Tree[T]): List[T] = {
    var queue: Queue[Tree[T]] = new Queue[Tree[T]]
    var result: List[T] = List()
    queue.addOne(t)
    while(!queue.isEmpty) {
      var node = queue.dequeue()
      result = result ++ List(node.element())
      if(node.isInstanceOf[Node[T]]) { queue.addAll(node.asInstanceOf[Node[T]].children) }
    }
    result
  } // Roughly 9 lines
...
// Be sure to make reduce as polymorphic as possible.
  ...
  def reduce[T] (f: (T,T) => T, b: T, L: List[T]): T = {
    L match {
      case List() => b
...
// function.
  ...
  def sumTreeA (t: Tree[A]): A = { 
    t match {
      case Leaf(x) => x
      case Node(x, children) => x + reduce((a:A, b:A) => a+b, new A(0), children.map(e => sumTreeA(e)))
    }
  } // Roughly 4 lines
