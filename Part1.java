...
class ASet<T extends LessThanOrEqualTo<T>> implements SetOf<T>, LessThanOrEqualTo<SetOf<T>> {
  ...
  public boolean lessThan(SetOf<T> s) {
    for (T e: s) {
      if (!this.member(e)) return true;
    }
    return false;
  }

  public boolean equalTo(SetOf<T> s) {
    return (this.subset(s) && s.subset(this)) ? true : false;
  }
  ...
  public boolean empty() {
    return (this.arrlist.size() == 0) ? true : false;
  }

  public boolean member(T t) {
    for (T e: this.arrlist) {
      if (e.equalTo(t)) return true;
    }
    return false;
  }
  ...
  public SetOf<T> union(SetOf<T> s) {
    ...
    result.arrlist = new ArrayList<T>(this.arrlist);
    for (T e: s) {
      if (!result.member(e)) {
        result.arrlist.add(e);
      }
    }
    return result;
  }
  ...
  public SetOf<T> difference(SetOf<T> s) {
    ...
    for (T a: this.arrlist) {
      ...
      if (unique) result.arrlist.add(a);
    }
    return result;
  }
...
class B extends A {
  ...
  public boolean lessThan(A o) {
    return (this.x < o.x) ? true : false;
  }
  public boolean equalTo(A o) {
    return (this.x == o.x) ? true : false;
  }

  public String toString() {
    return "B[" + (this.x - this.y) + "," + this.y + "]";
  }
}
...
class Part1 {
    ...
    static <T extends LessThanOrEqualTo<T>> SetOf<SetOf<T>> powerSet(SetOf<T> s) {
      ...
      SetOf<SetOf<T>> result = new ASet<SetOf<T>>();

      if (s.empty()) {
        result.add(new ASet<T>());;                //if no element then add an empty list into result
      }
      else {
        SetOf<T> x = new ASet<T>();
        T t = s.get();                             //take the first element
        x.add(t);                                  //add the first element into x ser
        SetOf<T> rest = s.difference(x);           //compute the rest of the set

        SetOf<SetOf<T>> powrest = powerSet(rest);  //compute the power set of the rest
        for(SetOf<T> e : powrest) {
          result.add(e);                           //add the original power set into result
          result.add(e.union(x));                  //add the union of the frist element and the the original power set into result
        }
      }
